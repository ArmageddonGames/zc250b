...
static void NES_titlescreen()
{
  ...
  try_zcmusic("zelda.nsf",0, ZC_MIDI_TITLE);
  clear_bitmap(screen);
  clear_bitmap(framebuf);
  init_NES_mode();
...
static void DX_mainscreen(int f)
{
  ...
  if(f<680+256 && (f&15)==0)
  {
    ...
    sprintf(tbuf, "%c2010 Armageddon Games",0xBC);
    //tbuf[0]=0xBC;
...
int readsaves(gamedata *savedata, PACKFILE *f)
{
  ...
  for (int i=0; i<save_count; i++)
  {
	...
	{
		if(!pfread(savedata[i].savedata, sizeof(long)*0x2000, f, true))
		{
			return 52;
		}
	}
	...
	{
		memset(savedata[i].savedata, 0, sizeof(long)*0x2000);
	}
  }
...
//  const char *passwd = datapwd;
  ...
  for(int i=0; iname[i]!='\0';iname[i]=='.'?iname[i]='\0':i++) { /* do nothing */ }
  strcat(iname,".icn");
...
newdata:
  system_pal();
  if(jwin_alert("Can't Find Saved Game File","It may be in use by another instance of ZC.","Create a new file from scratch?","Warning: Doing so will erase any previous saved games!","OK","Cancel",13,27,lfont)!=1)
  {
	exit(1);
  }
  game_pal();
...
reset:
  system_pal();
  if(jwin_alert("Can't Open Saved Game File","It may be in use by another instance of ZC.","Create a new file from scratch?","Warning: Doing so will erase any previous saved games!","OK","Cancel",13,27,lfont)!=1)
  {
	exit(1);
  }
  game_pal();
...
int writesaves(gamedata *savedata, PACKFILE *f)
{
  ...
  for (int i=0; i<MAXSAVES; i++)
  {
	...
	if(!pfwrite(savedata[i].savedata, sizeof(long)*0x2000, f))
	{
		return 51;
	}
  }
