...
enum
{
  ...
  cSLASHNEXTTOUCHY, cSLASHNEXTITEMTOUCHY, cBUSHNEXTTOUCHY, cEYEBALL_4, cTALLGRASSNEXT, cMAX
};
...
enum
{
  ...
  qr_NOFAIRYGUYFIRES /* Not for questmakers' use (yet?) */,
  qr_MAX
};
...
// Enemy misc2 types
enum { e2tNORMAL, e2tSPLITHIT, e2tSPLIT, e2tFIREOCTO, e2tBOMBCHU, e2tTRIBBLE,
// not implemented
e2tTRIBBLEWHISTLE, e2tLAST };
...
enum
{
  ...
  edefREFBALL, edefREFROCK, edefSTOMP, edefBYRNA,
...
public:
	...
	zctune(char _title[36], long _start, long _loop_start, long _loop_end, short _loop,short _volume, void *_data, byte _format) :start(_start), loop_start(_loop_start), loop_end(_loop_end), loop(_loop), volume(_volume), data(_data), format(_format)
	{
...
enum // used for gamedata ITEMS
{
  ...
  itype_spinscroll2, itype_quakescroll2, itype_agony, itype_stompboots,
  itype_whimsicalring, itype_perilring, itype_misc,
...
typedef struct gamedata
{
  ...
  // member functions
	...
	void set_savevalue(int pos, long value) { savedata[pos]=value;}
	long get_savevalue(int pos) {return savedata[pos];}
