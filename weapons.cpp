...
{
  x=X; y=Y; z=Z; id=Id; type=Type; power=pow; parentitem=Parentitem; dir=zc_max(Dir,0);
  ...
  switch(id)
  {
    case wSword: // Link's sword
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_sword, 1);
      }
      if (itemid >-1)
    ...
    case wWand: // Link's wand, as well as the Cane itself
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_wand, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
        defaultw = wWAND;
      hxsz=16; hysz=16; //hysz=24;
      LOADGFX(defaultw);
      break;
    }
    case wHammer:
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_hammer, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
    ...
    case wCByrna: // The Cane's beam
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_cbyrna, 1);
      }
      if (itemid >-1)
      ...
      clk = (int)((((2*type*PI)/qty)-(PI/2))*speed);
      type = 0;
      if (parentitem>-1)
      {
    ...
    case wWind:
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_whistle, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
    ...
    case wRefBeam:
      ...
      switch(dir)
      {
        ...
        case left:  flip=1;
        case right: /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); hxofs=2; hxsz=12; yofs = playing_field_offset+(BSZ ? 3 : 1); break;
      }
      if (id==wRefBeam)
      {
    ...
    case wArrow:
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_arrow, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
      ...
      switch(dir)
      {
        ...
        case up:    hyofs=2; hysz=12; break;
        case left:  flip=1;
        case right: /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); yofs=playing_field_offset + 1; hyofs=2; hysz=14; hxofs=2; hxsz=12; break;
      }
      if (itemid >-1)
    ...
    case wFire:
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_candle, 1);
      }
      switch(type) // Will be expanded later, maybe. --L
      {
        case 0:
        case 1:
          if (itemid>-1 && !isDummy)
          {
            defaultw = itemsbuf[itemid].wpn3; break;
          }
        default:
          defaultw = wFIRE;
      }
  ...
  case wBomb:
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_bomb, 1);
      }
      if (itemid >-1)
      {
        defaultw = itemsbuf[itemid].wpn;
  ...
  case wSBomb:
    {
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_sbomb, 1);
      }
      if (parentitem>-1)
      {
        defaultw = itemsbuf[itemid].wpn;
    ...
    case wBait:
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_bait, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
    ...
    case wMagic:
    {
      ...
      switch(dir)
      {
        ...
        case up:    hyofs=2; hysz=12; break;
        case left:  flip=1;
        case right: /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); hxofs=2; hxsz=12; break;
      }
      break;
    }
    case wBrang:
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_brang, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
    ...
    case wHookshot:
      ...
      if(isDummy || itemid<0)
      {
        itemid = getItemID(itemsbuf, itype_hookshot, 1);
      }
      if (itemid >-1)
        defaultw = itemsbuf[itemid].wpn;
      else
      ...
      switch(dir)
      {
        ...
        case left:  flip=1; /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); xofs+=2; yofs=playing_field_offset+4; hxofs=2; hxsz=12; break;
        case right: /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); xofs-=2; yofs=playing_field_offset+4; hxofs=2; hxsz=12; break;
      }
      break;
    case wHSHandle:
      step = 0;
    ...
    case wHSChain:
      {
        ...
        switch(dir)
        {
          case down:  LOADGFX(defaultw); xofs+=4;  yofs-=7;  break;
          case up:    LOADGFX(defaultw); xofs-=5;  yofs+=11; break;
          case left:  LOADGFX(defaultw); xofs+=10; yofs=playing_field_offset+4;  break;
          case right: LOADGFX(defaultw); xofs-=10; yofs=playing_field_offset+4;  break;
        }
      }
      break;
    case ewLitBomb:
    case ewBomb:
      ...
      switch(dir)
      {
        ...
        case up:    break;
        case left:  flip=1;
        case right: /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); break;
      }
      break;
    case ewLitSBomb:
    case ewSBomb:
    ...
    case ewArrow:
      ...
      switch(dir)
      {
        ...
        case left:  flip=1;
        case right: /*tile=o_tile+((frames>1)?frames:1)*/update_weapon_frame(((frames>1)?frames:1),o_tile); yofs=playing_field_offset+1; break;
      }
      break;
    case ewSword:
      LOADGFX(ewSWORD);
...
bool weapon::animate(int )
{
  ...
  if(id>wEnemyWeapons && id!=ewBrang && id != ewFireball2)
  switch(misc)
  {
	case up:    y-=.5; break;
	case down:  y+=.5; break;
  ...
  switch(id)
  {
      ...
      if(get_bit(quest_rules,qr_SWORDMIRROR))
      {
        ...
        if (hitcombo(checkx,checky,cMAGICPRISM))
        {
	      ...
	      w->hyofs = w->hxofs = 0;
              ...
              {
                case down:  w->flip=2;
                case up:    w->tile = w->o_tile; w->hyofs=2; w->hysz=12; break;
                case left:  w->flip=1;
                case right: w->tile=w->o_tile+((w->frames>1)?w->frames:1); w->hxofs=2; w->hxsz=12; break;
              }
              Lwpns.add(w);
            }
          }
    ...
    case wFire:
    {
      ...
      if(parentitem>-1 && itemsbuf[parentitem].family!=itype_book)
      {
        if(clk==32)
        {
          step=0;
        ...
        {
          dead=1;
          if(parentitem>-1 && !(itemsbuf[parentitem].flags & ITEM_FLAG2) && get_bit(quest_rules,qr_TEMPCANDLELIGHT) && (Lwpns.idCount(wFire) + Ewpns.idCount(ewFlame))==1)
          {
            isLit = false;
            checkLightSources();
          }
    ...
    case wSBomb:
    {
      ...
      {
        id = (id<wEnemyWeapons?((parentitem>-1 && itemsbuf[parentitem].family==itype_sbomb) ? wSBomb:wBomb):(id==ewLitSBomb||id==ewSBomb ? ewSBomb : ewBomb));
        hxofs=2000;
      }
      ...
      if(id<wEnemyWeapons)
      {
        if(clk==(misc-1))
        {
		  ...
		  for (int tx=-f1; tx<=f1; tx+=8) // -16,-8,0,8,16
          {
            ...
            {
			  findentrance(x+tx,y+ty+(tmpscr->flags7&fSIDEVIEW?2:0),mfBOMB,true);
			  if (id==wSBomb || id==wLitSBomb)
			  {
				findentrance(x+tx,y+ty+(tmpscr->flags7&fSIDEVIEW?2:0),mfSBOMB,true);
			  }
			  findentrance(x+tx,y+ty+(tmpscr->flags7&fSIDEVIEW?2:0),mfSTRIKE,true);
            }
          }
        }
    ...
    case ewBrang:
		{
			...
			if(misc==1)                                           // returning
			{
				...
				else
				{
					switch(dir)
					{
					case up:    if(y<misc2) killrang=true; break;
					case down:  if(y>misc2) killrang=true; break;
					case left:  if(x<misc2) killrang=true; break;
					case right: if(x>misc2) killrang=true; break;
					}
				}
			}
			else if (blocked())
...
reflect:
  ...
  switch(id)
  {
    ...
    case ewLitBomb:  step=0; misc=50; clk=misc-3;  hxofs=hyofs=-7;  hxsz=hysz=30; break;
    case ewSBomb: step=0; break;
    case ewLitSBomb: step=0; misc=50; clk=misc-3;  hxofs=hyofs=-16; hxsz=hysz=48; break;
...
void weapon::update_weapon_frame(int change, int orig)
{
	if(extend > 2)
	{
    if(orig/TILES_PER_ROW==(orig+txsz*change)/TILES_PER_ROW)
      tile=orig+txsz*change;
    else
      tile=orig+(txsz*change)+((tysz-1)*TILES_PER_ROW)*((orig+txsz*change)/TILES_PER_ROW)-(orig/TILES_PER_ROW);
	}
	else
	  tile=orig+change;
}
...
void weapon::draw(BITMAP *dest)
{
  ...
  if(frames)
  {
    if (id!=wBrang) // Brangs use clk2 for adjusting boomerang's flight range.
    {
      if(++clk2 >= o_speed)
      {
        clk2 = 0;
