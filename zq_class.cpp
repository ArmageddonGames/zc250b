...
int zmap::getCopyFFC() { return copyffc; }
...
int zmap::load(const char *path)
{
  ...
  mapscr tmpimportscr; //should initialize to zero
  ...
  for(int i=0; i<MAPSCRS; i++)
  {
    if(!pfread(&tmpimportscr,sizeof(mapscr),f,true))
      goto file_error;
	word databytes;
	if(!p_igetw(&databytes, f, true))
		goto file_error;

		tmpimportscr.data.resize( databytes, 0 );
		tmpimportscr.sflag.resize( databytes, 0 );
		tmpimportscr.cset.resize( databytes, 0 );

	if(!pfread( &tmpimportscr.data.front(), databytes*sizeof(word), f,true))
		goto file_error;
	if(!pfread( &tmpimportscr.sflag.front(), databytes, f,true))
		goto file_error;
	if(!pfread( &tmpimportscr.cset.front(), databytes, f,true))
		goto file_error;
...
int zmap::save(const char *path)
{
  ...
  for(int i=0; i<MAPSCRS; i++)
  {
	...
	int id = this->getCurrMap();
	int databytes = ZCMaps[id].tileWidth*ZCMaps[id].tileHeight;
	if(!p_iputw(databytes, f))
	{
		pack_fclose(f);
		return 3;
	}
	if(!pfwrite( &screens[i].data.front(), databytes*sizeof(word), f))
	{
		pack_fclose(f);
		return 4;
	}
	if(!pfwrite( &screens[i].sflag.front(), databytes, f))
	{
		pack_fclose(f);
		return 5;
	}
	if(!pfwrite( &screens[i].cset.front(), databytes, f))
	{
		pack_fclose(f);
		return 6;
	}
...
void copy_mapscr(mapscr *dest, const mapscr *src)
{
  ...
  for(int i=0; i<32; i++)
  {
    ...
    dest->lwpnclass[i]=src->lwpnclass[i];
    dest->ewpnref[i]=src->ewpnref[i];
    dest->ewpnclass[i]=src->ewpnclass[i];
    dest->guyref[i]=src->guyref[i];
    dest->guyclass[i]=src->guyclass[i];
    dest->ramref[i]=src->ramref[i];
  }
  for(int i=0; i<256; i++)
  ...
  dest->map_ffcref=src->map_ffcref;
    ...
    for (int k=0; k<2; k++)
    {
      if(LayerMaskInt[k+1]!=0)
      {
        ...
        if (layermap>-1 && layermap<map_count)
        {
          ...
          for (int i=0; i<176; i++)
          {
            put_walkflags(dest,((i&15)<<4)+x,(i&0xF0)+y,prv_mode?prvlayers[k].data[i]:TheMaps[layerscreen].data[i], k+1);
          }
        }
      }
    }
    ...
    for (int k=0; k<2; k++)
    {
      if(LayerMaskInt[k+1]!=0)
      {
        ...
        if (layermap>-1 && layermap<map_count)
        {
          ...
          for(int i=c; i<(c&0xF0)+16; i++)
          {
            put_walkflags(dest,((i&15)<<4)+x,y,TheMaps[layerscreen].data[i],k+1);
          }
        }
      }
    }
    ...
    for (int k=0; k<2; k++)
    {
      if(LayerMaskInt[k+1]!=0)
      {
        ...
        if (layermap>-1 && layermap<map_count)
        {
          ...
          for(int i=c; i<176; i+=16)
          {
            put_walkflags(dest,x,(i&0xF0)+y,TheMaps[layerscreen].data[i],k+1);
          }
        }
      }
    }
  ...
  if (flags&cWALK)
  {
    ...
    for (int k=0; k<2; k++)
    {
      if(LayerMaskInt[k+1]!=0)
      {
        ...
        {
          layerscreen=layermap*MAPSCRS+layer->layerscreen[k];
          put_walkflags(dest,x,y,TheMaps[layerscreen].data[c],k+1);
        }
      }
    }
  }
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    for(int i=0; i<map_count && i<MAXMAPS2; i++)
    {
      for (int j=0; j<MAPSCRS; j++)
      {
        ...
        if(!p_putc(TheMaps[scr].hasitem, f))
        {
          return qe_invalid;
        }
        for(int k=0;k<4;k++)
        ...
        if(!p_iputw(TheMaps[scr].color,f))
        {
          return qe_invalid;
        }
        ...
        for(int k=0;k<4;k++)
        {
          if(!p_iputw(TheMaps[scr].tilewarpdmap[k],f))
          {
            return qe_invalid;
          }
        }
        ...
        if(!p_putc(TheMaps[scr].tilewarpoverlayflags,f))
        {
          return qe_invalid;
        }
        ...
        if(!p_putc(TheMaps[scr].sidewarpoverlayflags,f))
        {
          return qe_invalid;
        }
        ...
        for(int k=0;k<4;k++)
        {
          if(!p_iputw(TheMaps[scr].sidewarpdmap[k],f))
          {
            return qe_invalid;
          }
        }
        ...
        if(!p_putc(TheMaps[scr].oceansfx,f))
        {
          return qe_invalid;
        }
        if(!p_putc(TheMaps[scr].bosssfx,f))
        {
          return qe_invalid;
        }
        if(!p_putc(TheMaps[scr].secretsfx,f))
        {
          return qe_invalid;
        }
        if(!p_putc(TheMaps[scr].holdupsfx,f))
        {
          return qe_invalid;
        }
        ...
        if(!p_putc(TheMaps[scr].lens_layer,f))
        {
          return qe_invalid;
        }
        ...
        for(int k=0;k<32;k++)
        {
          if((TheMaps[scr].numff>>k)&1)
          {
            ...
            if(!p_iputl(TheMaps[scr].ffx[k],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].ffy[k],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].ffxdelta[k],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].ffydelta[k],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].ffxdelta2[k],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].ffydelta2[k],f))
            {
              return qe_invalid;
            }
            ...
            if(!p_iputw(TheMaps[scr].ffscript[k],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][0],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][1],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][2],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][3],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][4],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][5],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][6],f))
            {
              return qe_invalid;
            }
            if(!p_iputl(TheMaps[scr].initd[k][7],f))
            {
              return qe_invalid;
            }
            if(!p_putc(TheMaps[scr].inita[k][0]/10000,f))
            {
              return qe_invalid;
            }
            if(!p_putc(TheMaps[scr].inita[k][1]/10000,f))
            {
              return qe_invalid;
            }
          }
...
int writeguys(PACKFILE *f, zquestheader *Header)
{
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    for(int i=0; i<eMAXGUYS; i++)
    {
      ...
      if(!p_iputw(guysbuf[i].misc1,f))
      {
        new_return(33);
      }
      if(!p_iputw(guysbuf[i].misc2,f))
      {
        new_return(34);
      }
      if(!p_iputw(guysbuf[i].misc3,f))
      {
        new_return(35);
      }
      if(!p_iputw(guysbuf[i].misc4,f))
      {
        new_return(36);
      }
      if(!p_iputw(guysbuf[i].misc5,f))
      {
        new_return(37);
      }
      if(!p_iputw(guysbuf[i].misc6,f))
      {
        new_return(38);
      }
      if(!p_iputw(guysbuf[i].misc7,f))
      {
        new_return(39);
      }
      if(!p_iputw(guysbuf[i].misc8,f))
      {
        new_return(40);
      }
      if(!p_iputw(guysbuf[i].misc9,f))
      {
        new_return(41);
      }
      if(!p_iputw(guysbuf[i].misc10,f))
      {
        new_return(42);
      }
      ...
      if(!p_iputw(guysbuf[i].misc11,f))
      {
        new_return(49);
      }
      if(!p_iputw(guysbuf[i].misc12,f))
      {
        new_return(50);
      }
    }
...
int save_quest(const char *filename, bool timed_save)
{
  ...
  if(compress)
  {
    ...
    //free(tmpfilename); // Double free() error - L
  }
...
int writeheader_210(PACKFILE *f, zquestheader *Header)
{
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writeheader_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
int writerules_210(PACKFILE *f, zquestheader *Header)
{
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writerules_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
int writedoorcombosets_210(PACKFILE *f, zquestheader *Header)
{
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writedoorcombosets_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
int writedmaps_210(PACKFILE *f, word version, word build, word start_dmap, word max_dmaps)
{
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writedmaps_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    for (int i=0; i<shops; i++)
    {
      for (int j=0; j<3; j++)
      {
      int temp = Misc->shop[i].item[j];
      if(Misc->shop[i].hasitem[j] == 0)
        temp=0;
        if(!p_putc(temp,f))
        {
          return qe_invalid;
    ...
    for (int i=0; i<9; i++)
    {
      if(!p_putc(Misc->warp[8].dmap[i],f))
      {
        return qe_invalid;
      }
      if(!p_putc(Misc->warp[8].scr[i],f))
      {
        return qe_invalid;
      }
    }
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writemisc_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writeitems_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writeweapons_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    for(int i=0; i<map_count && i<MAXMAPS2; i++)
    {
      for (int j=0; j<MAPSCRS; j++)
      {
        ...
        int temp = TheMaps[scr].item;
        if(TheMaps[scr].hasitem == 0)
        {
          temp=0;
        }
        if(!p_putc(temp,f))
        ...
        for(int k=0; k<10; k++)
        {
          int temp_enemy=TheMaps[scr].enemy[k];
          if (temp_enemy>10)
          {
            temp_enemy-=10;
          }
          if(!p_iputw(temp_enemy,f))
          {
            return qe_invalid;
          }
        }
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writemaps_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writecombos_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    if(!pfwrite(palnames,OLDMAXLEVELS*PALNAMESIZE,f))
    {
      new_return(6);
    }
    if(!p_iputw(palcycles,f))
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writecolordata_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    for (int i=0; i<msg_count; i++)
    {
      if(!pfwrite(MsgStrings[i].s,73/*sizeof(MsgStrings[0].s)*/,f))
      {
        return qe_invalid;
      }
      if(!p_iputw(MsgStrings[i].nextstring,f))
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writestrings_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writetiles_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }

  new_return(0);
}

int writemidis_210(PACKFILE *f)
{
  dword section_id=ID_MIDIS;
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    for(int i=0; i<MAXCUSTOMMIDIS; i++)
    {
      if(get_bit(midi_flags,i))
      {
        ...
        if(!write_midi((MIDI*)customtunes[i].data,f))
        {
          new_return(12);
        }
      }
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writemidis_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writecheats_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
int writeguys_210(PACKFILE *f, zquestheader *Header)
{
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writeguys_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
//  dword section_size=0;
  ...
  for (int writecycle=0; writecycle<2; ++writecycle)
  {
    ...
    //finally...  section data
    //GAH more old item code :( :( :(
    if(!p_putc(zinit.items[iRaft] ? 1 : 0,f))
    {
      new_return(5);
    }
    if(!p_putc(zinit.items[iRaft] ? 1 : 0,f))
    ...
    if(!p_putc(zinit.items[iBook] ? 1 : 0,f))
    {
      new_return(7);
    }
    if(!p_putc(zinit.items[iMKey] ? 1 : 0,f))
    {
      new_return(8);
    }
    if(!p_putc(zinit.items[iFlippers] ? 1 : 0,f))
    {
      new_return(9);
    }
    if(!p_putc(zinit.items[iBoots] ? 1 : 0,f))
    {
      new_return(10);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_ring),f))
    {
      new_return(11);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_sword),f))
    {
      new_return(12);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_shield),f))
    {
      new_return(13);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_wallet),f))
    {
      new_return(14);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_bracelet),f))
    {
      new_return(15);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_amulet),f))
    {
      new_return(16);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_bow),f))
    {
      new_return(17);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_candle),f))
    {
      new_return(18);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_brang),f))
    {
      new_return(19);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_arrow),f))
    {
      new_return(20);
    }

    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_potion),f))
    {
      new_return(21);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_whistle),f))
    {
      new_return(22);
    }
    if(!p_putc(zinit.bombs,f))
    ...
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_wand),f))
    {
      new_return(25);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_letter),f))

    {
      new_return(26);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_lens),f))
    {
      new_return(27);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_hookshot),f))
    {
      new_return(28);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_bait),f))
    {
      new_return(29);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_hammer),f))
    {
      new_return(30);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_dinsfire),f))
    {
      new_return(31);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_faroreswind),f))
    {
      new_return(32);
    }
    if(!p_putc(computeOldStyleBitfield(&zinit,itemsbuf,itype_nayruslove),f))
    {
      new_return(33);
    }
    //WTF is this cloak?
    ...
    if(!p_putc(itemsbuf[iSword].pickup_hearts,f))
    {
        new_return(47);
    }
    if(!p_putc(itemsbuf[iWSword].pickup_hearts,f))
    {
        new_return(47);
    }
    if(!p_putc(itemsbuf[iMSword].pickup_hearts,f))
    {
        new_return(47);
    }
    if(!p_putc(itemsbuf[iXSword].pickup_hearts,f))
    {
        new_return(47);
    }
    if(!p_putc(zinit.last_map,f))
    ...
    char tempbyte = (char)itemsbuf[iSword].misc1;
    if(!p_putc(tempbyte,f))
    {
        new_return(52);
    }
    tempbyte = (char)itemsbuf[iWSword].misc1;
    if(!p_putc(tempbyte,f))
    {
        new_return(52);
    }
    tempbyte = (char)itemsbuf[iMSword].misc1;
    if(!p_putc(tempbyte,f))
    {
        new_return(52);
    }
    tempbyte = (char)itemsbuf[iXSword].misc1;
    if(!p_putc(tempbyte,f))
    {
        new_return(52);
    }
    tempbyte = 0;
    tempbyte |= (itemsbuf[iSword].flags&ITEM_FLAG1) ? 1 : 0;
    tempbyte |= (itemsbuf[iWSword].flags&ITEM_FLAG1) ? 2 : 0;
    tempbyte |= (itemsbuf[iMSword].flags&ITEM_FLAG1) ? 4 : 0;
    tempbyte |= (itemsbuf[iXSword].flags&ITEM_FLAG1) ? 8 : 0;
    if(!p_putc(tempbyte,f)) //beam percent
    {
      new_return(53);
    }
    if(!p_putc((char)itemsbuf[iSword].misc2,f))
    {
        new_return(54);
    }
    if(!p_putc((char)itemsbuf[iWSword].misc2,f))
    {
        new_return(54);
    }
    if(!p_putc((char)itemsbuf[iMSword].misc2,f))
    {
        new_return(54);
    }
    if(!p_putc((char)itemsbuf[iXSword].misc2,f))
    {
        new_return(54);
    }
    if(!p_putc((char)itemsbuf[iHookshot].misc2,f)) //hookshot links
    {
      new_return(55);
    }
    if(!p_putc(zinit.msg_more_x,f))
  ...
  if (writesize!=int(section_size))
  {
    char ebuf[80];
    sprintf(ebuf, "%d != %d", writesize, int(section_size));
    jwin_alert("Error:  writeinitdata_210()","writesize != section_size",ebuf,NULL,"O&K",NULL,'k',0,lfont);
  }
...
int save_unencoded_quest_210(const char *filename, bool compressed)
{
  reset_combo_animations();
  reset_combo_animations2();
  strcpy(header.id_str,QH_NEWIDSTR);
  ...
  PACKFILE *f = pack_fopen_password(filename,compressed?F_WRITE_PACKED:F_WRITE, datapwd);
  if(!f)
    new_return(1);
  ...
  if (writedmaps_210(f,header.zelda_version,header.build,0,OLDMAXDMAPS)!=0)
  {
    new_return(6);
  }
  box_out("okay.");
  ...
  if (writecolordata_210(f,&misc,header.zelda_version,header.build,0,newerpdTOTAL)!=0)
  {
    new_return(12);
  }
  box_out("okay.");
