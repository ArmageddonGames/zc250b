...
{
  char *tmpfilename = temp_name(NULL);
...
int get_qst_buffers()
{
  ...
  if((MsgStrings=(MsgStr*)malloc(sizeof(MsgStr)*MAXMSGS))==NULL)
    return 0;
  ...
  Z_message("Allocating tile buffer (%s)... ", byte_conversion2(NEWMAXTILES*sizeof(tiledata),memrequested,-1,-1));
...
int readheader(PACKFILE *f, zquestheader *Header, bool keepdata)
{
  ...
  if(!strcmp(tempheader.id_str,QH_IDSTR))                      //pre-1.93 version
  {
    ...
    md5_starts( &ctx );
    md5_update( &ctx, (uint8 *) temp_pwd2, (unsigned long)strlen( temp_pwd2) );
...
int readdmaps(PACKFILE *f, zquestheader *Header, word, word, word start_dmap, word max_dmaps, bool keepdata)
{
  ...
  if (s_version>2)
  {
    ...
    {
    if( di[j/8] & (1 << (j%8)) ) tempDMap.disableditems[j]=1; else tempDMap.disableditems[j]=0;
    }
  }
  if(s_version >= 6)
  {
...
{
  word maxinfos=256; word maxshops=256;
  ...
  if (keepdata==true)
  {
    for(int i=0; i<MAXITEMS; i++)
    {
      ...
      if(s_version < 6)  // April 2007: Advanced item editing capabilities.
      {
        ...
        switch(i)
        {
          case iTriforce: tempitem.fam_type=1; break;
          case iBigTri: tempitem.fam_type=0; break;
          case iBombs:
            tempitem.fam_type=i_bomb; tempitem.power=4; tempitem.wpn=wBOMB; tempitem.wpn2=wBOOM; tempitem.misc1 = 50;
            if (get_bit(deprecated_rules,116)) tempitem.misc1 = 200; //qr_SLOWBOMBFUSES
            break;
          case iSBomb:
            tempitem.fam_type=i_sbomb; tempitem.power=16; tempitem.wpn=wSBOMB; tempitem.wpn2=wSBOOM; tempitem.misc1 = 50;
          ...
          case iSArrow:
            ...
            tempitem.power=4; tempitem.flags|=ITEM_GAMEDATA; tempitem.wpn=wSARROW;  break;
          case iGArrow:
            tempitem.wpn2 = get_bit(deprecated_rules,28) ? wGSPARKLE : 0; //qr_GASPARKLES
            tempitem.power=8; tempitem.flags|=(ITEM_GAMEDATA|ITEM_FLAG1); tempitem.wpn=wGARROW; break;
          case iBrang:
            tempitem.power=0; tempitem.wpn=wBRANG; tempitem.misc1=36; break;
          case iMBrang:
            tempitem.wpn2 = get_bit(deprecated_rules,29) ? wMSPARKLE : 0; //qr_MBSPARKLES
            tempitem.power=0; tempitem.wpn=wMBRANG; break;
          case iFBrang:
            tempitem.wpn3 = get_bit(deprecated_rules,30) ? wFSPARKLE : 0; //qr_FBSPARKLES
            tempitem.power=2; tempitem.wpn=wFBRANG; break;
          case iBoots:
            tempitem.magic = get_bit(deprecated_rules,51) ? 1 : 0;
            tempitem.power=7; break;
          case iWand:
            tempitem.magic = get_bit(deprecated_rules,49) ? 8 : 0;
            tempitem.power=2; tempitem.wpn=wWAND; tempitem.wpn3=wMAGIC; break;
          case iBCandle:
            tempitem.magic = get_bit(deprecated_rules,50) ? 4 : 0;
            tempitem.power=1; tempitem.flags|=(ITEM_GAMEDATA|ITEM_FLAG1); tempitem.wpn3=wFIRE; break;
          case iRCandle:
            tempitem.magic = get_bit(deprecated_rules,50) ? 4 : 0;
            tempitem.power=1; tempitem.wpn3=wFIRE; break;
          case iSword:
            tempitem.power=1; tempitem.flags|= ITEM_FLAG4 |ITEM_FLAG2; tempitem.wpn=tempitem.wpn3=wSWORD; tempitem.wpn2=wSWORDSLASH; break;
          case iWSword:
            tempitem.power=2; tempitem.flags|= ITEM_FLAG4 |ITEM_FLAG2; tempitem.wpn=tempitem.wpn3=wWSWORD; tempitem.wpn2=wWSWORDSLASH; break;
          case iMSword:
            tempitem.power=4; tempitem.flags|= ITEM_FLAG4 |ITEM_FLAG2; tempitem.wpn=tempitem.wpn3=wMSWORD; tempitem.wpn2=wMSWORDSLASH; break;
          case iXSword:
            tempitem.power=8; tempitem.flags|= ITEM_FLAG4 |ITEM_FLAG2; tempitem.wpn=tempitem.wpn3=wXSWORD; tempitem.wpn2=wXSWORDSLASH; break;
          case iNayrusLove:
            tempitem.flags |= get_bit(deprecated_rules,76) ? ITEM_FLAG1 : 0;
            tempitem.flags |= get_bit(deprecated_rules,75) ? ITEM_FLAG2 : 0;
            tempitem.wpn=wNAYRUSLOVE1A; tempitem.wpn2=wNAYRUSLOVE1B; tempitem.wpn3=wNAYRUSLOVES1A; tempitem.wpn4=wNAYRUSLOVES1B;
            tempitem.wpn6=wNAYRUSLOVE2A; tempitem.wpn7=wNAYRUSLOVE2B; tempitem.wpn8=wNAYRUSLOVES2A; tempitem.wpn9=wNAYRUSLOVES2B;
            tempitem.wpn5 = iwNayrusLoveShieldFront; tempitem.wpn10 = iwNayrusLoveShieldBack;
            tempitem.misc1=512; tempitem.magic=64; break;
          case iLens:
            tempitem.misc1=60;
            ...
            break;
          case iArrow:
            tempitem.power=2; tempitem.wpn=wARROW; break;
          case iHoverBoots:
            tempitem.misc1=45; tempitem.wpn=iwHover; break;
          case iDinsFire:
            tempitem.power=8; tempitem.wpn=wDINSFIRE1A; tempitem.wpn2=wDINSFIRE1B; tempitem.wpn3=wDINSFIRES1A; tempitem.wpn4=wDINSFIRES1B; tempitem.misc1 = 32; tempitem.misc2 = 200; tempitem.magic=32; break;
          case iFaroresWind:
            tempitem.magic=32; break;
          case iHookshot:
            tempitem.power=0; tempitem.flags|=ITEM_GAMEDATA; tempitem.wpn=wHSHEAD; tempitem.wpn2=wHSCHAIN_H; tempitem.wpn4=wHSHANDLE; tempitem.wpn3=wHSCHAIN_V; tempitem.misc1=50; tempitem.misc2=100; break;
          case iLongshot:
            tempitem.power=0; tempitem.wpn=wLSHEAD; tempitem.wpn2=wLSCHAIN_H; tempitem.wpn4=wLSHANDLE; tempitem.wpn3=wLSCHAIN_V;  tempitem.misc1=99; tempitem.misc2=100; break;
          case iHammer:
            tempitem.power=4; tempitem.wpn=wHAMMER; tempitem.wpn2=iwHammerSmack; break;
          case iCByrna:
            tempitem.power=1; tempitem.wpn=wCBYRNA; tempitem.wpn2=wCBYRNASLASH; tempitem.wpn3=wCBYRNAORB; tempitem.misc1=4; tempitem.misc2=16; tempitem.misc3=1; tempitem.magic=1; break;
          case iWhistle:
            tempitem.wpn=wWIND; tempitem.misc1=3; tempitem.flags|=ITEM_FLAG1; break;
          case iBRing:
            tempitem.power=2; tempitem.misc1=spBLUE; break;
          case iRRing:
            tempitem.power=4; tempitem.misc1=spRED; break;
          case iGRing:
            tempitem.power=8; tempitem.misc1=spGOLD; break;
          case iSpinScroll:
            tempitem.power = 2; tempitem.misc1 = 1; break;
          case iL2SpinScroll:
            tempitem.family=itype_spinscroll2; tempitem.fam_type=1;
            tempitem.magic=8; tempitem.power=2; tempitem.misc1 = 20; break;
          case iQuakeScroll:
            tempitem.misc1=0x10; tempitem.misc2=64; break;
          case iL2QuakeScroll:
            tempitem.family=itype_quakescroll2; tempitem.fam_type=1;
            tempitem.power = 2; tempitem.misc1=0x20; tempitem.misc2=192; tempitem.magic=8; break;
          case iChargeRing:
            tempitem.misc1=64; tempitem.misc2=128; break;
          case iL2ChargeRing:
            tempitem.misc1=32; tempitem.misc2=64; break;
          case iOldGlove:
          ...
          case iL2Bracelet:
            ...
            break;
          case iMKey:
            tempitem.power=0xFF; tempitem.flags |= ITEM_FLAG1;
            break;
        }
      }
      ...
      if(s_version < 8) // May 2007: Some corrections.
      {
        switch(i)
        {
          ...
          case iShield:
            ...
            // fallthrough
          case iSShield:
            tempitem.misc1|=shROCK|shARROW|shBRANG;
            ...
            break;
          case iWhispRing:
            tempitem.power=1; tempitem.flags|=ITEM_GAMEDATA|ITEM_FLAG1;
            tempitem.misc1 = 3; break;
          case iL2WhispRing:
            tempitem.power=0;
            tempitem.flags|=ITEM_GAMEDATA|ITEM_FLAG1; tempitem.misc1 = 3; break;
          case iL2Ladder:
          case iBow:
          case iCByrna:
            tempitem.power = 1; break;
            break;
        }
      }
      ...
      if(s_version < 12) // June 2007: More Misc. attributes.
      {
        ...
        switch(tempitem.family)
        {
          case itype_hoverboots:
            tempitem.usesound = WAV_ZN1HOVER; break;
          case itype_wand:
          case itype_book:
            tempitem.usesound = WAV_WAND; break;
          case itype_arrow:
            tempitem.usesound = WAV_ARROW; break;
          case itype_hookshot:
            tempitem.usesound = WAV_HOOKSHOT; break;
          case itype_brang:
            tempitem.usesound = WAV_BRANG; break;
          case itype_shield:
            tempitem.usesound = WAV_CHINK; break;
          case itype_sword:
            tempitem.usesound = WAV_SWORD; break;
          case itype_whistle:
            tempitem.usesound = WAV_WHISTLE; break;
          case itype_hammer:
            tempitem.usesound = WAV_HAMMER; break;
          case itype_dinsfire:
            tempitem.usesound = WAV_ZN1DINSFIRE; break;
          case itype_faroreswind:
            tempitem.usesound = WAV_ZN1FARORESWIND; break;
          case itype_nayruslove:
            tempitem.usesound = WAV_ZN1NAYRUSLOVE1; break;
          case itype_bomb:
          case itype_sbomb:
          case itype_quakescroll:
          case itype_quakescroll2:
            tempitem.usesound = WAV_BOMB; break;
          case itype_spinscroll:
          case itype_spinscroll2:
            tempitem.usesound = WAV_ZN1SPINATTACK; break;
        }
      }
...
void reset_itembuf(itemdata *item, int id)
{
  if (id<iLast)
  {
	...
	item->tile = tile; item->misc = miscs; item->csets = cset; item->frames = frames; item->speed = speed; item->delay = delay; item->ltm = ltm;
  }
}
...
int read_one_subscreen(PACKFILE *f, zquestheader *, bool keepdata, int i, word s_version, word )
{
  ...
  zc::auto_ptr<subscreen_object> ts_al(temp_sub, 2);

  //zc::auto_ptr<char> ts_dpl_al(NULL);
  ...
  if(s_version < 5)
  {
   ...
   // bool deletets = false;
    ...
    if(keepdata)
    {
		...
		temp_sub->dp1 = the_char_butcher.rape( char_length );
    ...
    if(s_version < 6)
    {
      switch(temp_sub->d1)
      {
      case ssiBOMB: temp_sub->d1 = itype_bomb; break;
      case ssiSWORD: temp_sub->d1 = itype_sword; break;
      case ssiSHIELD: temp_sub->d1 = itype_shield; break;
      case ssiCANDLE: temp_sub->d1 = itype_candle; break;
      case ssiLETTER: temp_sub->d1 = itype_letter; break;
      case ssiPOTION: temp_sub->d1 = itype_potion; break;
      case ssiLETTERPOTION: temp_sub->d1 = itype_letterpotion; break;
      case ssiBOW: temp_sub->d1 = itype_bow; break;
      case ssiARROW: temp_sub->d1 = itype_arrow; break;
      case ssiBOWANDARROW: temp_sub->d1 = itype_bowandarrow; break;
      case ssiBAIT: temp_sub->d1 = itype_bait; break;
      case ssiRING: temp_sub->d1 = itype_ring; break;
      case ssiBRACELET: temp_sub->d1 = itype_bracelet; break;
      case ssiMAP: temp_sub->d1 = itype_map; break;
      case ssiCOMPASS: temp_sub->d1 = itype_compass; break;
      case ssiBOSSKEY: temp_sub->d1 = itype_bosskey; break;
      case ssiMAGICKEY: temp_sub->d1 = itype_magickey; break;
      case ssiBRANG: temp_sub->d1 = itype_brang; break;
      case ssiWAND: temp_sub->d1 = itype_wand; break;
      case ssiRAFT: temp_sub->d1 = itype_raft; break;
      case ssiLADDER: temp_sub->d1 = itype_ladder; break;
      case ssiWHISTLE: temp_sub->d1 = itype_whistle; break;
      case ssiBOOK: temp_sub->d1 = itype_book; break;
      case ssiWALLET: temp_sub->d1 = itype_wallet; break;
      case ssiSBOMB: temp_sub->d1 = itype_sbomb; break;
      case ssiHCPIECE: temp_sub->d1 = itype_heartpiece; break;
      case ssiAMULET: temp_sub->d1 = itype_amulet; break;
      case ssiFLIPPERS: temp_sub->d1 = itype_flippers; break;
      case ssiHOOKSHOT: temp_sub->d1 = itype_hookshot; break;
      case ssiLENS: temp_sub->d1 = itype_lens; break;
      case ssiHAMMER: temp_sub->d1 = itype_hammer; break;
      case ssiBOOTS: temp_sub->d1 = itype_boots; break;
      case ssiDINSFIRE: temp_sub->d1 = itype_dinsfire; break;
      case ssiFARORESWIND: temp_sub->d1 = itype_faroreswind; break;
      case ssiNAYRUSLOVE: temp_sub->d1 = itype_nayruslove; break;
      case ssiQUIVER: temp_sub->d1 = itype_quiver; break;
      case ssiBOMBBAG: temp_sub->d1 = itype_bombbag; break;
      case ssiCBYRNA: temp_sub->d1 = itype_cbyrna; break;
      case ssiROCS: temp_sub->d1 = itype_rocs; break;
      case ssiHOVERBOOTS: temp_sub->d1 = itype_hoverboots; break;
      case ssiSPINSCROLL: temp_sub->d1 = itype_spinscroll; break;
      case ssiCROSSSCROLL: temp_sub->d1 = itype_crossscroll; break;
      case ssiQUAKESCROLL: temp_sub->d1 = itype_quakescroll; break;
      case ssiWHISPRING: temp_sub->d1 = itype_whispring; break;
      case ssiCHARGERING: temp_sub->d1 = itype_chargering; break;
      case ssiPERILSCROLL: temp_sub->d1 = itype_perilscroll; break;
      case ssiWEALTHMEDAL: temp_sub->d1 = itype_wealthmedal; break;
      case ssiHEARTRING: temp_sub->d1 = itype_heartring; break;
      case ssiMAGICRING: temp_sub->d1 = itype_magicring; break;
      case ssiSPINSCROLL2: temp_sub->d1 = itype_spinscroll2; break;
      case ssiQUAKESCROLL2: temp_sub->d1 = itype_quakescroll2; break;
      case ssiAGONY: temp_sub->d1 = itype_agony; break;
      case ssiSTOMPBOOTS: temp_sub->d1 = itype_stompboots; break;
      case ssiWHIMSICALRING: temp_sub->d1 = itype_whimsicalring; break;
      case ssiPERILRING: temp_sub->d1 = itype_perilring; break;

      default: temp_sub->d1 += itype_custom1 - ssiMAX;
      }
    }
    //fall-through
      default:
    ...
    //if(deletets) delete[] (char *)temp_sub->dp1; //wtf
  }
...
int readffscript(PACKFILE *f, zquestheader *Header, bool keepdata)
{
  ...
  if(s_version>1)
  {
	...
	else
	{
		...
		if(globalscripts[3]!=NULL) delete [] globalscripts[3];
		globalscripts[3] = new ffscript[1];
		globalscripts[3][0].command = 0xFFFF;
	}
    for(int i=0;i<3;i++)
    {
...
const char *old_sfx_string[Z35] =
{
  ...
  "Sword tapping (secret)", "Whistle whirlwind"
};
...
int readguys(PACKFILE *f, zquestheader *Header, bool keepdata)
{
  ...
  if (guyversion > 3)
  {
    ...
    for(int i=0; i<eMAXGUYS; i++)
    {
      ...
      if(!p_igetw(&(tempguy.misc1),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc2),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc3),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc4),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc5),f,keepdata))
      {
        return qe_invalid;
      }
      ...
      if(!p_igetw(&(tempguy.misc6),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc7),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc8),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc9),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc10),f,keepdata))
      {
        return qe_invalid;
      }
	  ...
	  if(guyversion >= 19)
	  {
      if(!p_igetw(&(tempguy.misc11),f,keepdata))
      {
        return qe_invalid;
      }
      if(!p_igetw(&(tempguy.misc12),f,keepdata))
      {
        return qe_invalid;
      }
  }
...
//          tempguy.misc6 = 8-(i<eGLEEOK1F?((i-eGLEEOK1)*40):((i-eGLEEOK1F)*40)); //neck offset from first body tile
          tempguy.misc6 = 8; //neck offset from first body tile
//          tempguy.misc7 = (i<eGLEEOK1F?(4-(i-eGLEEOK1)):(4-(i-eGLEEOK1F)))*40; //head offset from first body tile
          tempguy.misc7 = 40; //offset for each subsequent neck tile from the first neck tile
          tempguy.misc8 = 160; //head offset from first body tile
          tempguy.misc9 = 220; //flying head offset from first body tile
...
{
  "skip_header", "skip_rules", "skip_strings", "skip_misc", "skip_tiles", "skip_combos", "skip_comboaliases", "skip_csets", "skip_maps", "skip_dmaps", "skip_doors", "skip_items", "skip_weapons", "skip_colors", "skip_icons", "skip_initdata", "skip_guys", "skip_linksprites", "skip_subscreens", "skip_ffscript", "skip_sfx", "skip_midis", "skip_cheats", "skip_itemdropsets"
};
...
//  char percent_done[30];
  ...
  bool oldquest = false;
  int open_error=0;
  PACKFILE *f=open_quest_file(&open_error, filename, NULL, compressed, encrypted, show_progress);
  if(!f)
    return open_error;
  int ret=0;
