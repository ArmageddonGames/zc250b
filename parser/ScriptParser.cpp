...
ScriptsData *ScriptParser::assemble(IntermediateData *id)
{
	...
	vector<Opcode *> gainit = id->globalasInit;
	...
	it = scripts.find("Continue");
	if(it != scripts.end() && scripttypes["Continue"] == ScriptParser::TYPE_GLOBAL)
	{
		//append
		//get label
		int label = funcs[scripts["Continue"]][0]->getLabel();
		gainit.push_back(new OGotoImmediate(new LabelArgument(label)));
	}
	rval->theScripts["~Init"] = assembleOne(ginit, funcs,0);
	rval->scriptTypes["~Init"] = ScriptParser::TYPE_GLOBAL;
	rval->theScripts["~Continue"] = assembleOne(gainit, funcs,0);
	rval->scriptTypes["~Continue"] = ScriptParser::TYPE_GLOBAL;
	...
	for(vector<Opcode *>::iterator it2 = gainit.begin(); it2 != gainit.end(); it2++)
	{
		delete *it2;
	}
