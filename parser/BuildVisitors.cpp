...
// For the new arrays, declared in the script RAM
void BuildOpcodes::caseArrayDecl(ASTArrayDecl &host, void *param)
{
	...
	if(host.getList())
	{
		...
		for(list<ASTFloat *>::iterator it = host.getList()->getList().begin(); it != host.getList()->getList().end(); it++)
		{
			result.push_back(new OAddImmediate(new VarArgument(INDEX2), new LiteralArgument(10000)));
			t = (*it)->parseValue();
			t2 = ScriptParser::parseLong(t);
			if(!t2.second) printErrorMsg(*it, CONSTTRUNC, (*it)->getValue());
			result.push_back(new OSetImmediate(new VarArgument(RAMtype), new LiteralArgument(t2.first)));
		}
	}
}
