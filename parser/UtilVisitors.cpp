...
void Clone::caseArrayDecl(ASTArrayDecl &host, void *param)
{
	...
	if(host.getList())
	{
		host.getList()->execute(*this,param);
		l = (ASTArrayList *)result;
	}
	host.getSize()->execute(*this,param);
	ASTFloat *s = (ASTFloat *)result;
	result = new ASTArrayDecl(t,host.getName(),s,l,host.getLocation());
}
void Clone::caseArrayList(ASTArrayList &host, void *param)
{
	ASTArrayList *l = new ASTArrayList(host.getLocation());
	for(list<ASTFloat *>::iterator it = host.getList().begin(); it != host.getList().end(); it++)
	{
		(*it)->execute(*this,param);
		ASTFloat *v = (ASTFloat *)result;
		l->addParam(v);
	}
	result = l;
}
...
void GetScripts::caseScript(ASTScript &host, void *param) {void *temp; temp=&host; param=param; /*these are here to bypass compiler warnings about unused arguments*/}
